		...
		case "subcategories":
			if in.IsNull() {
				...
				for !in.IsDelim('}') {
					...
					in.WantColon()
					var v47 float32
					v47 = float32(in.Float32())
					(out.Subcategories)[key] = v47
					in.WantComma()
				}
				in.Delim('}')
			}
...
func easyjson3e1fa5ecEncodeTryOnInternalPkgDomain19(out *jwriter.Writer, in ClothesClassificationModelResponse) {
	...
	if len(in.Tags) != 0 {
		...
		out.RawString(prefix[1:])
		{
			out.RawByte('{')
			v50First := true
			for v50Name, v50Value := range in.Tags {
				if v50First {
					v50First = false
				} else {
					out.RawByte(',')
	...
	if len(in.Categories) != 0 {
		...
		}
		{
			out.RawByte('{')
			v51First := true
			for v51Name, v51Value := range in.Categories {
				if v51First {
					v51First = false
				} else {
					out.RawByte(',')
	...
	if len(in.Subcategories) != 0 {
		...
		}
		{
			out.RawByte('{')
			v52First := true
			for v52Name, v52Value := range in.Subcategories {
				if v52First {
					v52First = false
				} else {
					out.RawByte(',')
	...
	if len(in.Seasons) != 0 {
		...
		}
		{
			out.RawByte('{')
			v53First := true
			for v53Name, v53Value := range in.Seasons {
				if v53First {
					v53First = false
				} else {
					out.RawByte(',')
	...
	if len(in.Styles) != 0 {
		...
		}
		{
			out.RawByte('{')
			v54First := true
			for v54Name, v54Value := range in.Styles {
				if v54First {
					v54First = false
				} else {
					out.RawByte(',')
	...
	for !in.IsDelim('}') {
		...
		case "seasons":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v56 Season
					v56 = Season(in.String())
					out.Seasons = append(out.Seasons, v56)
					in.WantComma()
				}
				in.Delim(']')
			}
